...
select *
from sep_oct
where Tip_amount = (select MAX(Tip_amount) as mxtip from sep_oct)
--Q4
...
from sep_oct
--Q9
select avg(1.0 * Passenger_count)
from sep_oct
--Q10
select distinct datepart(hh,lpep_pickup_datetime), AVG(1.0 * Passenger_count) OVER(PARTITION by datepart(hh,lpep_pickup_datetime))
from sep_oct
--Q11
select distinct 
...
from sep_oct
where
Total_amount = (select max(Total_amount) from sep_oct where driver_id = 1) or 
Total_amount = (select min(Total_amount) from sep_oct where driver_id = 1)
--
select lpep_pickup_datetime, Total_amount, Passenger_count
from (select top 1 *
    from sep_oct
...
where driver_id not in (select driver_id from sep_oct)
--
select driver_id
from nyc_sample_data_for_sql_oct_2015
EXCEPT
...
where not exists (select distinct driver_id from sep_oct
                where driver_id = oct.driver_id)
--Q25
select oct_rev - sep_rev
from (select (select sum(total_amount) from sep_oct) sep_rev,
...
from sep_oct sep, nyc_sample_data_for_sql_oct_2015 oct
...
from nyc_sample_data_for_sql_oct_2015 oct
JOIN sep_oct sep on sep.trip_id = oct.trip_id
