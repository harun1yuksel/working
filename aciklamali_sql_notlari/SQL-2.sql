...
# Bunun için en az 3 faz gerekiyor. Bu fazları uygulayıp veriyi bölmemiz gerekiyor

# 1.NF
# 1.Sütunların satırlarla kesişen alanda tek hücrenin tek bir bilgi içermesini sağlamak
# 2.Sadece 1 primary key i olmalı
# 3.SQL-1 de tekrarlayan satır olmamalı demiştik bir de
...
# 1.Tekrarlayan satır içermemesi  .. YOK
...
# Transitive Dependency: Bir non-key attribute unun başka bir non-key attribute unu bağlılığı olmaması
# Bağımlılık ne demek?
"""
1       a
1       a
...
Eğer 1. sütun 2. sütuna bağımlı olsaydı ; 1. sütunda 4 değerinin gelmesinin sebebi c derdik ama
.. c HER geldiğinde 4 değeri gelmiyor 5 değeri de gelebiliyor. ANCAK 2. sütun 1. sütuna bağlı 
ÇÜNKÜ; 1 her geldiğinde a gelmiş , 2 her geldiğince e gelmiş, 4 her geldiğinde(Gerçi burada 1 tane 4 değeri var) c gelmiş. 
.. O yüzden 2. sütundaki değerlerin gelmesi 1. sütuna bağlı diyoruz. Tersten bakarsak, c her geldiğinde 4 gelmemiş bazen 5 gelmiş
"""
...
"""
TCKNO   KITAP_ID  KİTAP ADI  YAZAR (excel satır 55-64)
...
# Burada "KITAP_ADI" ve "YAZAR" ı farklı tablolarda tutabilirim.(Not yazar için aynı şey geçerli değil ama "Kitap_adi" için 
# .. başka tabloya aldığımda ayrı id verip bunu kitap_id ile bağlayabilirim)

# ÖNEMLİ NOT: 1. Partial Dependency, 2. Transitivity Dependency.
# Partial: 1 primary key(2 sütundan oluşan), bir de non-key attibutelar varsa, non-key attribute lar primary key in
# .. bir alt kümesi olan(sadece 1 sütun) ile identify edilirse partial. Ama 2 si ile birlikte(PK daki değişkenler) ifade ediliyorsa
# .. sorun yok(Yani diğer tabloya almama gerek yok). Diğer türlü partial lik varsa non-key i diğer tabloya almalıyım onu identify eden
# .. ile ama onu identify eden sütun hala eski yerinde kalıp aynı zamanda başka adla diğer sütunda da durabilir. 
# ÖRN: 5 sütun var, ilk 2 si primary key(Yani composite pk), 3. sütun , 2. sütun ile take başına identify edilebiliyor.
# .. O yüzden partial dependent diyoruz. Eğer pk lardan biri ile degilde ancak 2 si ile identify ediliyor olsaydı sorun yoktu
# .. Partial dependency olduğu için 3. sütunu başka bir tabloya almam lazım
# Transitivity: bir non key attribute' un başka bir non key attribute' a bağımlı olmaması 
# .. sadece ve sadece tek bir primary key' e bağımlı olması gerekiyordu
# ÖRN: 4 sütun var. 1 primary key var(1. sütun). 3. sütun, 2. sütun ile identify edilebiliyor. O zaman 3. sütunu başka tabloya almalıyım

# Diagrama bakalım şimdi(RDMS diagramı). 
# İş yerinde departmanlarla vs konuştuk ettik ve harita oluşturduk buna göre database i oluşturacağız
# Tablolar: Mail, Person, Phone , Loan, Book, Author, Publisher
"""
person  \                      / author
mail   --  Loan <---> Book    -- publisher
phone  /

"""

# Phone içine niye maili koymadık, publisher ın içine neden book u koymadık vs? 
# .. Üstteki yaptıklarımıza(Faz adımlarına) ve domain bilgisine göre karar verdik
...
ntext             0 chars    1.073.741  chars      2 times the string length

char(10)   : 10 karakterlik yer ayırır  karakter verilerini tutuyor .. char(10) "ali" yi 3 karakteri alır ve 
... 7 karakteride boş olarak tutar
varchar(10): En fazla 10 karakterlik yer ayırır ama az karakter yazsakta olur  karakter verilerini tutuyor
... char(10) "ali" yi 3 karakteri alır ve 7 karakteri tutmaz
nvarchar   : ülkelerin dillerine göre karakterler kullanmak istiyorsak
"""
